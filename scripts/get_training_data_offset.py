...
parser.add_argument("-offs","--offset", type=float, help="Maximum offset from center in radians")
...
def get_cutouts(ra,dec,p_map,width=10.0*utils.arcmin,res=res, max_offset=5.0*utils.arcmin):
    # max_offset is in radians and gives the maximum amount that the thumbnail will be off center. For 
    # a max_offset of half of the width (converted to radians), the furthest
    # that the cutout may be off-center is half the width in the ascension direction and half the width
    # in the declination direction
    ra,dec = ra*u.deg,dec*u.deg
    cutouts = reproject.thumbnails(p_map,(dec.to(u.radian).value+max_offset*uniform(-1,1),ra.to(u.radian).value+max_offset*uniform(-1,1)),r=width,res=res)
    return cutouts
...
with jsonlines.open(ofn,mode="w") as write_file:
    for i in range(osize):
        ...
        for k in list_freqs:
            cutout = get_cutouts(ra[i],dec[i],maps_dict[k]["p_map"],width=width,res=res,max_offset=max_offset)
            cutout = cutout[:64,:64]
